...
<div class="card basic-data-table">
    <div class="card-header">
        <div class="row align-items-center justify-content-between">
            <div class="col-auto">
                <h5 class="card-title mb-0">Pasca Persalinan <?= esc($ibu['ibuRM']) ?> (<?= esc($ibu['ibuNama']) ?>)</h5>
            </div>
            <div class="col-auto">
                <div class="d-flex align-items-center gap-3 mt-20">
                    ...
                    <a href="<?= site_url('add-pasca-persalinan/' . esc($ibu['ibuID'])) ?>" class="btn btn-primary-600 radius-8 px-20 py-11 d-flex align-items-center gap-2">
    ...
    <div class="card-body">
        <table class="table bordered-table mb-0" id="dataTable" data-page-length='10'>
            ...
            <tbody>
                <?php foreach ($pascaPersalinans as $pascaPersalinan) : ?>
                    <tr>
                        <td>
                            <div class="form-check style-check d-flex align-items-center">
                                <input class="form-check-input" type="checkbox">
                                <label class="form-check-label">
                                    <?= esc($pascaPersalinan['pemeriksaanRutinID']) ?>
                                </label>
                            </div>
                        </td>
                        <td><?= esc($pascaPersalinan['tanggalPemeriksaan']) ?></td>
                        <td><?= esc($pascaPersalinan['jamPemeriksaan']) ?></td>
                        <td><?= esc($pascaPersalinan['tekananDarah']) ?></td>
                        <td><?= esc($pascaPersalinan['nadi']) ?></td>
                        <td>
                            <a href="/view-pasca-persalinan/<?= esc($pascaPersalinan['pemeriksaanRutinID']) ?>" class="w-32-px h-32-px bg-info-focus text-info-main rounded-circle d-inline-flex align-items-center justify-content-center">
                                <iconify-icon icon="lucide:eye"></iconify-icon>
                            </a>
                            <a href="/edit-pasca-persalinan/<?= esc($pascaPersalinan['pemeriksaanRutinID']) ?>" class="w-32-px h-32-px bg-success-focus text-success-main rounded-circle d-inline-flex align-items-center justify-content-center">
                                <iconify-icon icon="lucide:edit"></iconify-icon>
                            </a>
                            <a type="button" onclick="confirmDelete('<?= esc($pascaPersalinan['pemeriksaanRutinID']) ?>')" class="w-32-px h-32-px bg-danger-focus text-danger-main rounded-circle d-inline-flex align-items-center justify-content-center">
                                <iconify-icon icon="mingcute:delete-2-line"></iconify-icon>
                            </a>
                            <form id="deleteForm_<?= esc($pascaPersalinan['pemeriksaanRutinID']) ?>" action="<?= site_url('delete-pasca-persalinan') ?>" method="post">
                                <input type="hidden" name="pemeriksaanRutinID" value="<?= esc($pascaPersalinan['pemeriksaanRutinID']) ?>">
                            </form>
                        </td>
                    </tr>
                <?php endforeach; ?>
...
<script>
    ...
    function confirmDelete(pemeriksaanRutinID) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $('#deleteForm_' + pemeriksaanRutinID).submit();
            }
        });
    }
</script>
