            ...
            <tbody>
                <?php foreach ($kamars as $kamar) : ?>
                    <tr>
                        ...
                        <td>
                            <a href="/edit-kamars/<?= esc($kamar['kamarID']) ?>" class="w-32-px h-32-px bg-success-focus text-success-main rounded-circle d-inline-flex align-items-center justify-content-center">
                                <iconify-icon icon="lucide:edit"></iconify-icon>
                            </a>
                            <a type="button" onclick="confirmDelete('<?= esc($kamar['kamarID']) ?>')" class="w-32-px h-32-px bg-danger-focus text-danger-main rounded-circle d-inline-flex align-items-center justify-content-center">
                                <iconify-icon icon="mingcute:delete-2-line"></iconify-icon>
                            </a>
                            <form id="deleteForm_<?= esc($kamar['kamarID']) ?>" action="<?= site_url('delete-kamars') ?>" method="post">
...
<script>
    ...
    function confirmDelete(kamarID) {
        Swal.fire({
