...
<div class="card basic-data-table">
    <div class="card-header">
        <div class="row align-items-center justify-content-between">
            <div class="col-auto">
                <h5 class="card-title mb-0">Antenatal <?= esc($ibu['ibuRM']) ?> (<?= esc($ibu['ibuNama']) ?>)</h5>
            </div>
            <div class="col-auto">
                <div class="d-flex align-items-center gap-3 mt-20">
                    ...
                    <a href="<?= site_url('add-antenatals/' . esc($ibu['ibuID'])) ?>" class="btn btn-primary-600 radius-8 px-20 py-11 d-flex align-items-center gap-2">
                        <iconify-icon icon="mingcute:table-2-line" class="text-xl"></iconify-icon> Antenatal Baru
                    </a>
                </div>
            </div>
        </div>
    ...
    <div class="card-body">
        <table class="table bordered-table mb-0" id="dataTable" data-page-length='10'>
            ...
            <tbody>
                <?php foreach ($antenatals as $antenatal) : ?>
                    <tr>
                        <td>
                            <div class="form-check style-check d-flex align-items-center">
                                <input class="form-check-input" type="checkbox">
                                <label class="form-check-label">
                                    <?= esc($antenatal['antenatalID']) ?>
                                </label>
                            </div>
                        </td>
                        <td><?= esc($antenatal['keluhanSekarang']) ?></td>
                        <td><?= esc($antenatal['tekananDarah']) ?></td>
                        <td><?= esc($antenatal['beratBadan']) ?></td>
                        <td><?= esc($antenatal['umurKehamilan']) ?></td>
                        <td>
                            <a href="/view-antenatals/<?= esc($antenatal['antenatalID']) ?>" class="w-32-px h-32-px bg-info-focus text-info-main rounded-circle d-inline-flex align-items-center justify-content-center">
                                <iconify-icon icon="lucide:eye"></iconify-icon>
                            </a>
                            <a href="/edit-antenatals/<?= esc($antenatal['antenatalID']) ?>" class="w-32-px h-32-px bg-success-focus text-success-main rounded-circle d-inline-flex align-items-center justify-content-center">
                                <iconify-icon icon="lucide:edit"></iconify-icon>
                            </a>
                            <a type="button" onclick="confirmDelete('<?= esc($antenatal['antenatalID']) ?>')" class="w-32-px h-32-px bg-danger-focus text-danger-main rounded-circle d-inline-flex align-items-center justify-content-center">
...
<script>
    ...
    function confirmDelete(antenatalID) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $('#deleteForm_' + antenatalID).submit();
            }
        });
    }
</script>
