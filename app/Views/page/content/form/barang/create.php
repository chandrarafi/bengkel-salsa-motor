...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-20">
    <h6 class="fw-semibold mb-0 text-lg">Tambah Barang Baru</h6>
    <ul class="d-flex align-items-center gap-2 text-sm">
        ...
        <li class="text-secondary-light">-</li>
        <li class="fw-medium text-secondary-light">Tambah Barang</li>
    </ul>
</div>
...
<div class="card radius-12 border">
    <div class="card-header border-bottom border-neutral-200 px-20 py-14 d-flex align-items-center justify-content-between">
        <h6 class="card-title mb-0 text-base fw-bold">Formulir Tambah Barang / Sparepart</h6>
    ...
    <div class="card-body p-24">
        <form id="createBarangForm" action="<?= site_url('admin/barang/store') ?>" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
            <?= csrf_field() ?>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-8">
                    <div class="row g-3">
                        <!-- Kode Barang -->
                        <div class="col-md-6">
                            <label class="form-label text-xs fw-semibold text-neutral-700 mb-1">Kode Barang <span class="text-danger-600">*</span></label>
                            <div class="icon-field">
                                <span class="icon">
                                    <iconify-icon icon="solar:qr-code-bold-duotone"></iconify-icon>
                                </span>
                                <input type="text" name="kode" id="kode" class="form-control form-control-sm radius-8 <?= isset($errors['kode']) ? 'is-invalid' : '' ?>" value="<?= old('kode', $autoKode) ?>" placeholder="Contoh: BRG0000001" maxlength="10" required>
                            </div>
                            <div class="invalid-feedback <?= isset($errors['kode']) ? 'd-block' : '' ?>" id="kode_feedback"><?= esc($errors['kode'] ?? 'Kode barang wajib diisi.') ?></div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="icon-field">
                                ...
                                <input type="text" name="nama_barng" id="nama_barng" class="form-control form-control-sm radius-8 <?= isset($errors['nama_barng']) ? 'is-invalid' : '' ?>" value="<?= old('nama_barng') ?>" placeholder="Contoh: Oli Yamalube Super Sport" maxlength="50" required>
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="icon-field">
                                ...
                                <select name="idkategori" id="idkategori" class="form-select form-select-sm radius-8 <?= isset($errors['idkategori']) ? 'is-invalid' : '' ?>" required>
                                    <option value="" disabled <?= old('idkategori') ? '' : 'selected' ?>>-- Pilih Kategori --</option>
                                    <?php foreach ($kategori as $kat): ?>
                                        <option value="<?= $kat['idkategori'] ?>" <?= old('idkategori') == $kat['idkategori'] ? 'selected' : '' ?>><?= esc($kat['namakategori']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="icon-field">
                                ...
                                <select name="idsatuan" id="idsatuan" class="form-select form-select-sm radius-8 <?= isset($errors['idsatuan']) ? 'is-invalid' : '' ?>" required>
                                    <option value="" disabled <?= old('idsatuan') ? '' : 'selected' ?>>-- Pilih Satuan --</option>
                                    <?php foreach ($satuan as $sat): ?>
                                        <option value="<?= $sat['idsatuan'] ?>" <?= old('idsatuan') == $sat['idsatuan'] ? 'selected' : '' ?>><?= esc($sat['nama_satuan']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="icon-field">
                                ...
                                <input type="number" name="stok" id="stok" class="form-control form-control-sm radius-8 <?= isset($errors['stok']) ? 'is-invalid' : '' ?>" value="<?= old('stok') ?>" placeholder="Contoh: 25" min="0" required>
                            </div>
                ...
                <div class="col-lg-4">
                    <div class="card h-100 p-0 border radius-12 overflow-hidden">
                        <div class="card-header border-bottom bg-base py-16 px-24">
                          <h6 class="text-md fw-semibold mb-0">Upload Foto Barang</h6>
                        </div>
                        <div class="card-body p-24">
                            <div class="upload-image-wrapper d-flex align-items-center gap-3">
                              <div class="uploaded-img d-none position-relative h-120-px w-120-px border input-form-light radius-8 overflow-hidden border-dashed bg-neutral-50">
                                ...
                                <img id="uploaded-img__preview" class="w-100 h-100 object-fit-cover" src="" alt="image">
                              </div>
            ...
            <div class="d-flex align-items-center justify-content-end gap-2">
                ...
                <button type="submit" id="submitBtn" class="btn btn-primary-600 radius-8 px-24 py-8 text-sm d-flex align-items-center gap-2 fw-semibold">
                    <iconify-icon icon="mingcute:save-2-fill" class="text-base"></iconify-icon> Simpan Barang
                </button>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // AJAX Form Submit
        $('#createBarangForm').on('submit', function(e) {
            e.preventDefault();
            var form = this;
