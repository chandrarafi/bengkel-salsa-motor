...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-20">
    <div class="d-flex align-items-center gap-3">
        ...
        <h6 class="fw-semibold mb-0 text-lg">Edit Transaksi Servis #<?= esc($header['faktur']) ?></h6>
    </div>
    <ul class="d-flex align-items-center gap-2 text-sm">
        ...
        <li class="text-secondary-light">-</li>
        <li class="fw-medium text-secondary-light">Edit Transaksi</li>
    </ul>
</div>

<form id="updateServisForm" action="<?= site_url('admin/transaksiservis/update/' . esc($header['faktur'])) ?>" method="post">
    <?= csrf_field() ?>
        ...
        <div class="col-lg-6">
            <div class="card radius-12 border h-100">
                ...
                <div class="card-body p-20">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label text-xs fw-semibold text-neutral-700 mb-1">No. Faktur Servis</label>
                            <input type="text" class="form-control form-control-sm radius-8 fw-bold text-primary-600 bg-neutral-50" value="<?= esc($header['faktur']) ?>" readonly>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label text-xs fw-semibold text-neutral-700 mb-1">Tanggal Transaksi <span class="text-danger-600">*</span></label>
                            <input type="date" name="tglfaktur" id="tglfaktur" class="form-control form-control-sm radius-8" value="<?= esc($header['tglfaktur']) ?>" required>
                        ...
                        <div class="col-12">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="text" name="nama_pelanggan" id="nama_pelanggan" class="form-control radius-start-8" value="<?= esc($header['nama_pelanggan']) ?>">
                                ...
                                <button type="button" class="btn btn-neutral-200 text-neutral-800 fw-bold px-12 text-xs" id="btnPelangganUmum">
                                    Umum
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="merkkendaraan" id="merkkendaraan" class="form-control form-control-sm radius-8" value="<?= esc($header['merkkendaraan']) ?>" required>
                            <div class="invalid-feedback" id="merkkendaraan_feedback"></div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="nopol" id="nopol" class="form-control form-control-sm radius-8 font-mono text-uppercase" value="<?= esc($header['nopol']) ?>" required>
                            <div class="invalid-feedback" id="nopol_feedback"></div>
                        </div>

                        <div class="col-12">
                            <label class="form-label text-xs fw-semibold text-neutral-700 mb-1">Keluhan / Diagnosa Perbaikan</label>
                            <textarea name="alasan" id="alasan" rows="6" class="form-control form-control-sm radius-8" style="min-height: 150px;"><?= esc($header['alasan']) ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-6">
            <div class="card radius-12 border h-100">
                <div class="card-header border-bottom border-neutral-200 px-20 py-14 d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <h6 class="card-title mb-0 text-base fw-bold">Tambah / Ulik Item Transaksi</h6>
    ...
    <div class="d-flex align-items-center justify-content-end gap-2 mb-30">
        <a href="<?= site_url('admin/transaksiservis/show/' . esc($header['faktur'])) ?>" class="border border-neutral-400 bg-neutral-100 text-neutral-700 text-sm px-20 py-10 radius-8 d-flex align-items-center gap-2 fw-medium">
        ...
        <button type="submit" id="btnSubmitFinal" class="btn btn-primary-600 radius-8 px-24 py-10 text-sm d-flex align-items-center gap-2 fw-semibold">
            <iconify-icon icon="mingcute:save-2-fill" class="text-base"></iconify-icon> Simpan Perubahan Transaksi
        </button>
    </div>
</form>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#updateServisForm').on('submit', function(e) {
            e.preventDefault();
            var form = this;

            Swal.fire({
                title: 'Simpan Perubahan Transaksi Servis?',
                text: 'Apakah perubahan data transaksi servis ini sudah benar?',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    processUpdateServis(form);
                }
            });
        });

        function processUpdateServis(form) {
            var formData = $(form).serialize();
            var $btn = $('#btnSubmitFinal');
            var originalBtnHtml = $btn.html();

            $('.form-control').removeClass('is-invalid');
            $btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm" role="status"></span> Memperbarui Transaksi...');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Transaksi Servis Diperbarui!',
                ...
                error: function() {
                    $btn.prop('disabled', false).html(originalBtnHtml);
                    Swal.fire({ icon: 'error', title: 'Error', text: 'Gagal memperbarui transaksi servis.' });
                }
            });
        }
    });
