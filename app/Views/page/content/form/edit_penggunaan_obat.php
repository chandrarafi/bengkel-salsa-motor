...
<div class="col-lg-12 mb-10">
    <div id="tambah-card" class="card">
        <div class="card-header">
            <h5 class="card-title mb-0">Edit Penggunaan Obat <?= $penggunaanObat['penggunaanObatID'] ?></h5>
        </div>
        <div class="card-body">
            <form action="" method="post">
                <div class="row">
                    <input type="hidden" id="penggunaanObatID" name="penggunaanObatID" value="<?= esc($penggunaanObat['penggunaanObatID']) ?>">

                    <div class="col-md-6 mb-10">
                        <label class="form-label">Pilih Ibu</label>
                        <div class="has-validation">
                            <input type="hidden" id="ibuID" name="ibuID" value="<?= esc($penggunaanObat['ibuID']) ?>">
                            <div class="input-group">
                                <input type="text" id="ibuNama" name="ibuNama" class="form-control" placeholder="Pilih Data Ibu" value="<?= esc($penggunaanObat['ibuNama']) ?>" readonly>
                    ...
                    <div class="col-md-12 mb-20">
                        ...
                        <textarea name="catatan" class="form-control" placeholder="Masukan Keterangan Komplikasi" required><?= esc($penggunaanObat['catatan']) ?></textarea>
                    </div>
                </div>
                <div class="col-md-12">
                ...
                <div class="col-md-12">
                    <div class="table-responsive scroll-sm">
                        <table class="table bordered-table text-sm" id="detail-obat-table">
                            ...
                            <tbody>
                                <?php $no = 01;
                                foreach ($detailPenggunaanObat as $dpo) { ?>
                                    <tr class="data-detail-obat">
                                        <td><?= $no++; ?></td>
                                        <td>
                                            <input type="hidden" id="penggunaanObatID" class="detailPenggunaanObatID" name="detailPenggunaanObatID[]" value="<?= esc($dpo['detailPenggunaanObatID']) ?>">
                                            <select class="obat-select form-control select2-basic" required>
                                                <option value="" disabled selected>Pilih Obat</option>
                                                <?php foreach ($obats as $obat) { ?>
                                                    <option data-stok="<?= esc($obat['obatStok']) ?>" <?= ($obat['obatID'] == $dpo['obatID']) ? "selected" : "" ?> data-harga="<?= esc($obat['obatHarga']) ?>" value="<?= esc($obat['obatID']) ?>"><?= esc($obat['obatNama']) ?></option>
                                                <?php } ?>
                                            </select>
                                        </td>
                                        <td class="obatHarga"><?= esc($dpo['obatHarga']) ?></td>
                                        <td class="obatStok"><?= esc($dpo['obatStok']) ?></td>
                                        <td><input type="number" class="obatQty form-control" value="<?= esc($dpo['obatJumlah']) ?>" min="0"></td>
                                        <td class="obatSubtotal"><?= esc($dpo['obatJumlah'] * $dpo['obatHarga']) ?></td>
    ...
    (function($) {
        $('#addRow').click(function() {
            ...
            <tr class="data-detail-obat">
                ...
                <td>
                    <input type="hidden" class="detailPenggunaanObatID" name="detailPenggunaanObatID[]" value="">
                    <select class="obat-select form-control select2-basic" required>
                        <option value="" disabled selected>Pilih Obat</option>
                        <?php foreach ($obats as $obat) { ?>
                ...
                <td class="obatHarga"></td>
                <td class="obatStok"></td>
                <td><input type="number" class="obatQty form-control" min="0"></td>
                <td class="obatSubtotal"></td>
                <td class="text-center">
                    <button type="button" class="remove-row"><iconify-icon icon="ic:twotone-close" class="text-danger-main text-xl"></iconify-icon></button>
                </td>
            </tr>
        ...
        $('form').submit(function(event) {
            ...
            const formData = {
                ...
                penggunaanObatID: $('#penggunaanObatID').val(),
                details: []
            };

            $('#detail-obat-table tbody tr').each(function() {
                const row = $(this);
                const detailPenggunaanObatID = row.find('.detailPenggunaanObatID').val() || null;
                formData.details.push({
                    detailPenggunaanObatID: detailPenggunaanObatID,
            ...
            $.ajax({
                url: '<?= site_url("update-penggunaan-obat") ?>',
                type: 'POST',
                data: formData,
