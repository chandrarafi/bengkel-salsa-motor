...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-20">
    <h6 class="fw-semibold mb-0 text-lg">Edit Transaksi Penjualan Barang</h6>
...
<form id="updatePenjualanForm" action="<?= site_url('admin/penjualan/update/' . esc($header['faktur'])) ?>" method="post">
    <?= csrf_field() ?>
            ...
            <div class="card radius-12 border h-100">
                ...
                <div class="card-body p-20">
                    <div class="row g-3">
                        <!-- No Faktur (Readonly) -->
                        <div class="col-12">
                            <label class="form-label text-xs fw-semibold text-neutral-700 mb-1">No. Faktur Penjualan</label>
                            <input type="text" name="faktur" id="faktur" class="form-control form-control-sm radius-8 fw-bold text-primary-600 bg-neutral-100" value="<?= esc($header['faktur']) ?>" readonly>
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <div class="input-group">
                                <input type="text" name="nama_pelanggan" id="nama_pelanggan" class="form-control form-control-sm radius-start-8 fw-semibold" value="<?= esc($header['nama_pelanggan']) ?>" placeholder="Nama pelanggan..." required>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="keterangan" id="keterangan" class="form-control form-control-sm radius-8" rows="3" style="min-height: 80px;" placeholder="Catatan transaksi..."><?= esc($header['keterangan']) ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#updatePenjualanForm').on('submit', function(e) {
            e.preventDefault();
            var form = this;
            ...
            $btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm" role="status"></span> Menyimpan Perubahan...');
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        Swal.fire({
                            ...
                            window.location.href = response.redirect_url || '<?= site_url('admin/penjualan') ?>';
                        });
                    } else {
                        if (response.errors) {
                            $.each(response.errors, function(field, msg) {
