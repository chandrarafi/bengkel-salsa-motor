...
<head>
    ...
    <title><?= esc($title ?? 'Laporan Data Barang') ?></title>
    <style>
        * {
...
<body>
    ...
    <div class="no-print-bar">
        <a href="<?= site_url('admin/laporan/barang') ?>" class="btn-action btn-back">
    ...
    <div class="report-paper">
        ...
        <div class="report-title-block">
            <h2 class="report-title">LAPORAN DATA BARANG & SPAREPART</h2>
            <p class="report-period">
                <strong>Kategori:</strong> 
                <?php if (!empty($selectedKategori)): ?>
                    <?= esc($selectedKategori['namakategori']) ?>
                <?php else: ?>
        ...
        <table class="report-table">
            <thead>
                <tr>
                    ...
                    <th class="text-end" style="width: 90px;">Harga Beli</th>
                    <th class="text-end" style="width: 90px;">Harga Jual</th>
                    <th class="text-center" style="width: 50px;">Stok</th>
                    <th class="text-end" style="width: 110px;">Total Asset (Beli)</th>
                </tr>
            </thead>
            <tbody>
                <?php if (!empty($barang)): $no = 1; foreach ($barang as $item): 
                ...
                ?>
                    <tr>
                        ...
                        <td><?= esc($item['nama_barng']) ?></td>
                        <td><?= esc($item['namakategori'] ?: '-') ?></td>
                        <td><?= esc($item['nama_satuan'] ?: '-') ?></td>
                        <td class="text-end">Rp <?= number_format($hargaBeli, 0, ',', '.') ?></td>
                        <td class="text-end">Rp <?= number_format($hargaJual, 0, ',', '.') ?></td>
                        <td class="text-center fw-bold"><?= $stok ?></td>
                        <td class="text-end fw-bold">Rp <?= number_format($subAsset, 0, ',', '.') ?></td>
                    </tr>
                <?php endforeach; else: ?>
                    <tr>
                        <td colspan="9" class="text-center" style="padding: 20px;">Tidak ada data barang yang ditemukan untuk periode ini.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
            <tfoot>
                <tr class="tfoot-summary">
                    ...
                    <td class="text-end">-</td>
                    <td class="text-center"><?= number_format($totalStok ?? 0, 0, ',', '.') ?></td>
                    <td class="text-end">Rp <?= number_format($totalNilaiBeli ?? 0, 0, ',', '.') ?></td>
                </tr>
            </tfoot>
        </table>
        ...
        <div class="signature-block">
            <div style="width: 300px;">
                <p style="margin: 0 0 6px 0; font-weight: bold;">Catatan / Ringkasan:</p>
                <ul style="margin: 0; padding-left: 18px; font-size: 11px; color: #475569;">
                    <li>Total Jenis Barang: <strong><?= number_format($totalItems ?? 0, 0, ',', '.') ?> item</strong></li>
                    <li>Total Quantity Stok: <strong><?= number_format($totalStok ?? 0, 0, ',', '.') ?> pcs</strong></li>
                    <li>Total Asset (Modal Beli): <strong>Rp <?= number_format($totalNilaiBeli ?? 0, 0, ',', '.') ?></strong></li>
                    <li>Potensi Omset (Harga Jual): <strong>Rp <?= number_format($totalNilaiJual ?? 0, 0, ',', '.') ?></strong></li>
                </ul>
            </div>
