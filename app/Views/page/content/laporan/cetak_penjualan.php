...
<head>
    ...
    <title><?= esc($title ?? 'Laporan Penjualan Barang') ?></title>
    <style>
        * {
...
<body>
    ...
    <div class="no-print-bar">
        <a href="<?= site_url('admin/laporan/penjualan') ?>" class="btn-action btn-back">
            &larr; Kembali ke Laporan Penjualan
        </a>
    ...
    <div class="report-paper">
        ...
        <div class="report-title-block">
            <h2 class="report-title">LAPORAN TRANSAKSI PENJUALAN BARANG (POS)</h2>
            <p class="report-period">
                ...
                <?php else: ?>
                    Semua Transaksi Penjualan (s/d <?= date('d F Y') ?>)
                <?php endif; ?>
            </p>
        </div>
        ...
        <table class="report-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 120px;">No. Struk/Faktur</th>
                    <th class="text-center" style="width: 80px;">Tanggal</th>
                    <th style="width: 110px;">Pelanggan</th>
                    <th>Rincian Barang Terjual</th>
                    <th class="text-center" style="width: 50px;">Qty</th>
                    <th class="text-end" style="width: 110px;">Total Penjualan (Rp)</th>
                </tr>
            </thead>
            <tbody>
                                ...
                                <ul style="margin: 0; padding-left: 14px; font-size: 10.5px;">
                                    <?php foreach ($details as $d): ?>
                                        <li>
                                            ...
                                            (<strong><?= esc($d['jumlah']) ?></strong> <?= esc($d['nama_satuan'] ?: 'pcs') ?> &times; Rp <?= number_format($d['detailhargajual'], 0, ',', '.') ?> = Rp <?= number_format($d['subtotal'], 0, ',', '.') ?>)
                                        </li>
                                    <?php endforeach; ?>
                                </ul>
                            <?php else: ?>
                ...
                <?php endforeach; else: ?>
                    <tr>
                        <td colspan="7" class="text-center" style="padding: 20px;">Tidak ada data transaksi penjualan untuk periode ini.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
            <tfoot>
                <tr class="tfoot-summary">
                    <td colspan="5" class="text-end">TOTAL KESELURUHAN (OMSET PENJUALAN):</td>
        ...
        <div class="signature-block">
            <div style="width: 300px;">
                <p style="margin: 0 0 6px 0; font-weight: bold;">Ringkasan Penjualan:</p>
                <ul style="margin: 0; padding-left: 18px; font-size: 11px; color: #475569;">
                    <li>Total Transaksi Struk: <strong><?= number_format($totalFaktur ?? 0, 0, ',', '.') ?> Transaksi</strong></li>
                    <li>Total Quantity Barang Terjual: <strong><?= number_format($totalQtyAll ?? 0, 0, ',', '.') ?> pcs</strong></li>
                    <li>Total Omset Penjualan: <strong>Rp <?= number_format($totalNominal ?? 0, 0, ',', '.') ?></strong></li>
                </ul>
            </div>
