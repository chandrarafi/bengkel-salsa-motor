...
<head>
    ...
    <title><?= esc($title ?? 'Laporan Transaksi Servis') ?></title>
    <style>
        * {
...
<body>
    ...
    <div class="no-print-bar">
        <a href="<?= site_url('admin/laporan/transaksiservis') ?>" class="btn-action btn-back">
            &larr; Kembali ke Laporan Transaksi Servis
        </a>
    ...
    <div class="report-paper">
        ...
        <div class="report-title-block">
            <h2 class="report-title">LAPORAN TRANSAKSI SERVIS BENGKEL (WORK ORDER)</h2>
            <p class="report-period">
                ...
                <?php else: ?>
                    Semua Transaksi Servis (s/d <?= date('d F Y') ?>)
                <?php endif; ?>
                <?php if (!empty($status)): ?>
                    | <strong>Status:</strong> <?= esc(ucfirst($status)) ?>
                <?php endif; ?>
            </p>
        </div>
        ...
        <table class="report-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 130px;">Pelanggan & Kendaraan</th>
                    <th>Rincian Jasa Servis & Sparepart</th>
                    <th class="text-end" style="width: 110px;">Total Servis (Rp)</th>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td>
                            <strong><?= esc($pelanggan) ?></strong><br>
                            <span style="font-size: 10px; color: #64748b;">
                                <?= esc($header['merkkendaraan'] ?: '-') ?> <?= !empty($header['nopol']) ? '(' . esc($header['nopol']) . ')' : '' ?>
                            </span>
                        </td>
                        <td>
                                    ...
                                    <?php foreach ($details as $d): ?>
                                        <li>
                                            <?php if (!empty($d['jenis_servis'])): ?>
                                                [Jasa] <strong><?= esc($d['jenis_servis']) ?></strong> (Rp <?= number_format($d['dettotaljual'], 0, ',', '.') ?>)
                                            <?php elseif (!empty($d['nama_barng'])): ?>
                                                [Part] <strong><?= esc($d['nama_barng']) ?></strong> (<?= esc($d['detjml']) ?> <?= esc($d['nama_satuan'] ?: 'pcs') ?> = Rp <?= number_format($d['dettotaljual'], 0, ',', '.') ?>)
                                            <?php else: ?>
                                                Item Detail
                                            <?php endif; ?>
                                        </li>
                            ...
                            <?php else: ?>
                                <em>- Tidak ada rincian item -</em>
                            <?php endif; ?>
                        </td>
                        <td class="text-end fw-bold">Rp <?= number_format($total, 0, ',', '.') ?></td>
                        <td class="text-center"><strong><?= esc(ucfirst($st)) ?></strong></td>
                    </tr>
                <?php endforeach; else: ?>
                    <tr>
                        <td colspan="7" class="text-center" style="padding: 20px;">Tidak ada data transaksi servis untuk periode ini.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
            <tfoot>
                <tr class="tfoot-summary">
                    <td colspan="5" class="text-end">TOTAL OMSET / PENDAPATAN SERVIS:</td>
                    <td class="text-end">Rp <?= number_format($totalNominal ?? 0, 0, ',', '.') ?></td>
                    <td class="text-center"><?= number_format($totalFaktur ?? 0, 0, ',', '.') ?> Faktur</td>
                </tr>
            </tfoot>
        </table>
        ...
        <div class="signature-block">
            <div style="width: 300px;">
                <p style="margin: 0 0 6px 0; font-weight: bold;">Ringkasan Pendapatan Servis:</p>
                <ul style="margin: 0; padding-left: 18px; font-size: 11px; color: #475569;">
                    <li>Total Transaksi Servis (Faktur): <strong><?= number_format($totalFaktur ?? 0, 0, ',', '.') ?> Faktur</strong></li>
                    <li>Total Omset Pendapatan Servis: <strong>Rp <?= number_format($totalNominal ?? 0, 0, ',', '.') ?></strong></li>
                </ul>
            </div>
