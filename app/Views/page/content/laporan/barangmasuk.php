...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-20">
    <h6 class="fw-semibold mb-0 text-lg">Laporan Transaksi Barang Masuk</h6>
    <ul class="d-flex align-items-center gap-2 text-sm">
        ...
        <li class="text-secondary-light">-</li>
        <li class="fw-medium text-secondary-light">Laporan Barang Masuk</li>
    </ul>
</div>
...
<div class="card radius-12 border mb-24">
    <div class="card-header border-bottom border-neutral-200 px-20 py-14 d-flex align-items-center justify-content-between">
        <h6 class="card-title mb-0 text-base fw-bold d-flex align-items-center gap-2">
            ...
            Filter Laporan Barang Masuk (Rentang Tanggal)
        </h6>
        <?php 
            ...
            $cetakUrl = site_url('admin/laporan/barangmasuk/cetak') . ($queryStr ? '?' . $queryStr : '');
        ?>
    ...
    <div class="card-body p-20">
        <form method="get" action="<?= site_url('admin/laporan/barangmasuk') ?>" class="row g-3 align-items-end">
            ...
            <div class="col-md-4 col-lg-6 d-flex align-items-center gap-2">
                ...
                <?php if (!empty($tgl_awal) || !empty($tgl_akhir)): ?>
                    <a href="<?= site_url('admin/laporan/barangmasuk') ?>" class="btn btn-outline-neutral-400 text-neutral-700 radius-8 px-16 py-8 text-sm d-flex align-items-center gap-1">
...
<div class="card basic-data-table radius-12 border">
    <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2 px-20 py-14 border-bottom border-neutral-200">
        <h6 class="card-title mb-0 text-base fw-bold">Daftar Transaksi Barang Masuk <?= (!empty($tgl_awal) || !empty($tgl_akhir)) ? '(Terfilter)' : '' ?></h6>
        <div class="d-flex align-items-center gap-3 text-xs text-secondary-light">
            ...
            <span>|</span>
            <span>Total Pembelian: <strong class="text-success-main fw-bold">Rp <?= number_format($totalNominal ?? 0, 0, ',', '.') ?></strong></span>
        </div>
    </div>
    <div class="card-body p-20">
            ...
            <table class="table bordered-table align-middle text-sm mb-0" id="dataTable" data-page-length="10" style="width:100%;">
                <thead>
                    <tr>
                        ...
                        <th scope="col">Rincian Barang & Quantity</th>
                        <th scope="col" class="text-center" style="width: 80px;">Total Qty</th>
                        <th scope="col" class="text-end" style="width: 120px;">Total Harga (Rp)</th>
                                        ...
                                        <?php foreach ($details as $d): ?>
                                            <div class="d-flex align-items-center justify-content-between text-xs border-bottom pb-1 mb-1">
                                                ...
                                                <span class="text-neutral-700">
                                                    <strong><?= esc($d['jumlah']) ?></strong> <?= esc($d['nama_satuan'] ?: 'pcs') ?> &times; Rp <?= number_format($d['detailhargabeli'], 0, ',', '.') ?> = <strong class="text-success-main">Rp <?= number_format($d['subtotal'], 0, ',', '.') ?></strong>
                                                </span>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
        ...
        if ($('#dataTable').length) {
            new DataTable('#dataTable', {
                language: {
                    ...
                    zeroRecords: "Tidak ada transaksi barang masuk yang cocok",
