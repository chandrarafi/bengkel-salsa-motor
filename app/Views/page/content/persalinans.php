...
<div class="card basic-data-table">
    <div class="card-header">
        <div class="row align-items-center justify-content-between">
            <div class="col-auto">
                <h5 class="card-title mb-0">Persalinan <?= esc($ibu['ibuRM']) ?> (<?= esc($ibu['ibuNama']) ?>)</h5>
            </div>
            <div class="col-auto">
                <div class="d-flex align-items-center gap-3 mt-20">
                    ...
                    <a href="<?= site_url('add-persalinans/' . esc($ibu['ibuID'])) ?>" class="btn btn-primary-600 radius-8 px-20 py-11 d-flex align-items-center gap-2">
                        <iconify-icon icon="mingcute:table-2-line" class="text-xl"></iconify-icon> Persalinan Baru
                    </a>
                </div>
            </div>
        </div>
        ...
        <table class="table bordered-table mb-0" id="dataTable" data-page-length='10'>
            <thead>
                <tr>
                    ...
                    <th scope="col">Jam Persalinan</th>
                    <th scope="col">Cara Persalinan</th>
                    <th scope="col">Petugas Persalinan</th>
                    <th scope="col">Aksi</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($persalinans as $persalinan) : ?>
                    <tr>
                        <td>
                            <div class="form-check style-check d-flex align-items-center">
                                <input class="form-check-input" type="checkbox">
                                <label class="form-check-label">
                                    <?= esc($persalinan['persalinanID']) ?>
                                </label>
                            </div>
                        </td>
                        <td><?= esc($persalinan['tanggalPersalinan']) ?></td>
                        <td><?= esc($persalinan['jamPersalinan']) ?></td>
                        <td><?= esc($persalinan['caraPersalinan']) ?></td>
                        <td><?= esc($persalinan['userNama']) ?></td>
                        <td>
                            <a href="/view-persalinans/<?= esc($persalinan['persalinanID']) ?>" class="w-32-px h-32-px bg-info-focus text-info-main rounded-circle d-inline-flex align-items-center justify-content-center">
                                <iconify-icon icon="lucide:eye"></iconify-icon>
                            </a>
                            <a href="/edit-persalinans/<?= esc($persalinan['persalinanID']) ?>" class="w-32-px h-32-px bg-success-focus text-success-main rounded-circle d-inline-flex align-items-center justify-content-center">
                                <iconify-icon icon="lucide:edit"></iconify-icon>
                            </a>
                            <a type="button" onclick="confirmDelete('<?= esc($persalinan['persalinanID']) ?>')" class="w-32-px h-32-px bg-danger-focus text-danger-main rounded-circle d-inline-flex align-items-center justify-content-center">
...
<script>
    ...
    function confirmDelete(persalinanID) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $('#deleteForm_' + persalinanID).submit();
            }
        });
    }
</script>
